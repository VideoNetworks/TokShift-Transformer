...
class DropTime(nn.Module):
	def __init__(self, n_seg=8, prob=0.5):
		super(DropTime, self).__init__()
		self.drop_time = nn.Dropout3d(p=prob)
		self.n_seg = n_seg
		self.prob = prob
	...
	def forward(self, x):
		...
		x = x.view(b*n, c, t, 1, 1) # B*N, C, T, 1, 1
		x = self.drop_time(x)
		x = x.view(b, n, c, t) # B, N, C, T
		x = x.permute(0, 3, 1, 2).contiguous() # B, T, N, C
		x = x.view(b*t, n, c)
		return x
...
class Mlp(nn.Module):
	def __init__(self, config):
		...
		self.droptime = DropTime(prob=config.transformer["dropout_rate"])		
...
class Embeddings(nn.Module):
	...
	def __init__(self, config, img_size, in_channels=3):
		...
		#self.dropout = Dropout(config.transformer["dropout_rate"])
		self.dropout = DropTime(prob=config.transformer["dropout_rate"])
...
class Block(nn.Module):
	def __init__(self, config, vis):
		...
		print("Shift Configures n_segs {}, fold_div {}".format(self.n_seg,
									self.fold_div))
